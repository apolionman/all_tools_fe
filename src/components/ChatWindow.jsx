...
const ChatWindow = () => {
    ...
    const renderMarkdown = ({ node, inline, className, children, ...props }) => {
        ...
        return !inline && match ? (<SyntaxHighlighter style={vscDarkPlus} language={match[1]} PreTag="div" {...props}>
    ...
    return (<div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {messages.length === 0 ? (<div className="flex flex-col items-center justify-center h-full text-center px-4">
          ...
          </div>) : (<div className="max-w-3xl mx-auto w-full">
            {messages.map((msg) => (<div key={msg.id} className={`group w-full text-gray-800 dark:text-gray-100 border-b border-black/10 dark:border-gray-700 ${msg.role === 'user' ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-[#444654]'}`}>
                <div className="text-base gap-4 md:gap-6 md:max-w-2xl lg:max-w-2xl xl:max-w-3xl p-4 md:py-6 flex mx-auto">
                  {msg.role === 'assistant' && (<div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex-shrink-0 flex items-center justify-center">
                      <span className="text-white text-sm">AI</span>
                    </div>)}
                  {msg.role === 'user' && (<div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 flex-shrink-0 flex items-center justify-center">
                  ...
                  <div className="relative flex-grow min-w-0 flex flex-col">
                    ...
                    code: renderMarkdown,
                    p: ({ children }) => (<p className="prose dark:prose-invert prose-sm max-w-none mb-4 last:mb-0">{children}</p>),
        ...
        {/* Typing indicator */}
        {isTyping && (<div className="w-full bg-gray-50 dark:bg-[#444654] border-b border-black/10 dark:border-gray-700">
            <div className="text-base gap-4 md:gap-6 md:max-w-2xl lg:max-w-2xl xl:max-w-3xl p-4 md:py-6 flex mx-auto">
      ...
      <div className="sticky bottom-0 bg-white dark:bg-gray-800 border-t dark:border-gray-700 pt-2 pb-4 px-4">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-end rounded-xl bg-gray-100 dark:bg-gray-700/50 border dark:border-gray-700 shadow-sm">
            <textarea ref={inputRef} rows={1} className="flex-1 bg-transparent border-0 focus:ring-0 py-3 px-4 text-gray-800 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 resize-none max-h-32" value={input} onChange={handleInputChange} onKeyDown={(e) => {
            if (e.key === 'Enter' && !e.shiftKey) {
                e.preventDefault();
                sendMessage();
            }
        }} placeholder="Message AI..." disabled={isTyping}/>
            <button onClick={sendMessage} disabled={isTyping || !input.trim()} className={`m-2 p-2 rounded-full ${isTyping || !input.trim()
            ? 'text-gray-400'
